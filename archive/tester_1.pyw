...
SAVE_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "saved_data.json")
...
def bind_mousewheel(widget, command):
    def _on_mousewheel(event):
        # Windows и MacOS используют event.delta
        if event.delta:
            command(int(-1*(event.delta/120)))
        # Linux использует event.num (4 - вверх, 5 - вниз)
        else:
            if event.num == 4:
                command(-1)
            elif event.num == 5:
    ...
    widget.bind_all("<MouseWheel>", _on_mousewheel) # Для Windows/Mac
    widget.bind_all("<Button-4>", _on_mousewheel)   # Для Linux (вверх)
    widget.bind_all("<Button-5>", _on_mousewheel)   # Для Linux (вниз)
...
def bind_text_scroll(text_widget):
    ...
    def _bind_to_text(e):
        text_widget.bind_all("<MouseWheel>", _on_mousewheel_text)
        text_widget.bind_all("<Button-4>", _on_mousewheel_text)
        text_widget.bind_all("<Button-5>", _on_mousewheel_text)

    def _unbind_from_text(e):
        bind_mousewheel(canvas, lambda step: canvas.yview_scroll(step, "units"))

    # Перехват прокрутки при наведении на текстовое поле
    text_widget.bind("<Enter>", _bind_to_text)
    # Возврат прокрутки холста при уходе курсора
    text_widget.bind("<Leave>", _unbind_from_text)
